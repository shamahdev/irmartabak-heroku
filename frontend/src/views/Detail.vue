  ...
  <div class="detail">
      ...
      <div class="container-fluid row p-0 m-0 my-5">
        <div class="col-10 col-md-8 mx-auto my-auto p-0">
          ...
          <div>
            ...
            ><!--
          --><button class="btn btn-skeleton btn-lg px-5 py-3">
              Berikan Rating
            </button>
          </div>
    ...
    <vue-page-transition name="fade-in-up">
      <section
        ...
        id="detailmartabak"
        :key="martabak.id"
        v-for="martabak in martabakdata"
      >
        <div class="container-fluid row p-0 m-0 mb-5">
          <img
            class="thumbnail fit-cover"
            :src="martabak.image"
            :alt="martabak.name"
          />
        </div>
        <div class="container-fluid row p-0 m-0 my-5">
          <div
            class="col-10 col-md-8 mx-auto my-auto p-0"
            :key="rating.average"
            v-for="rating in ratingmartabak"
          >
            <div
              v-if="rating.object_id == martabak.id"
              :cm="(currentM = martabak.id)"
            >
              ...
              <div :rid="(giverating.rating = rating.id)">
                ...
                ></star-rating>
                <div class="h2 my-3">
                  <p class="inline-block text-dark mr-3">
                ...
                <modal id="startrating" title="Berikan Rating">
                  ...
                  <div class="p-2 mt-2 mb-3" v-if="already_rate">
                    Kamu telah memberikan rating
                    {{ userrating + " untuk " + martabak.name }}
                  </div>
                </modal>
                <!-- Modal2 -->
                <modal id="buymethod" title="Pilih Layanan Pemesanan">
                  <div class="row m-0">
                    <div class="col-12 mx-auto">
                      <p class="lead2 mb-4">
                        Pilih cabang yang paling dekat dengan lokasi kamu saat
                        ini!
                        <a
                          class="text-warning"
                    ...
                    <div class="col-12 mx-auto mb-4">
                      <p class="lead3 mt-3">
                        Tubagus Ismail Raya (GrabFood Only)
                      </p>
                      <div>
                        <a
              ...
              >
                ...
                <p>
                  {{
                    "Rp. " +
                      martabak.lowest_price +
                      " - Rp. " +
                      martabak.highest_price
                  }}
                </p>
                <label class="lead3">Rating</label>
                <p>{{ parseFloat(rating.average) + "/5" }}</p>
                <label class="lead3">Ukuran</label>
                <p v-if="martabak.size_besar">
                  Besar<koma v-if="martabak.size_kecil">, Kecil</koma>
                </p>
                <p v-else>Kecil</p>
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.$axios.get("/api/martabak/").then(response => {
        this.martabakdata = response.data.filter(m =>
          m.slug.includes(this.$route.params.name)
        );
      });
    ...
    rate() {
      this.$axios
        .post("/api/userrating/", this.giverating)
        .then(r => {})
        .catch(error => {
